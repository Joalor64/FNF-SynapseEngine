...
class Achievements
{
	...
	public static function reloadList()
	{
		...
		final modLoaded:String = Mods.currentModDirectory;
		Mods.currentModDirectory = null;
		loadAchievementJson(Paths.mods('achievements.json'));

		for (i => mod in Mods.parseList().enabled)
		{
			final achi:String = Paths.mods('$mod/achievements.json');
			if (FileSystem.exists(achi))
			{
				Mods.currentModDirectory = mod;
				loadAchievementJson(achi);
			}
		}
			...
			if (addMods && retVal != null)
			{
				for (i in 0...retVal.length)
				{
					...
					var data:Achievement = {
						name: achieve.name != null ? achieve.name : key,
						description: achieve.description != null ? achieve.description : "",
						hidden: achieve.hidden == true,
						maxScore: achieve.maxScore != null ? achieve.maxScore : 0,
						maxDecimals: achieve.maxDecimals != null ? achieve.maxDecimals : 0
					};

					createAchievement(key, data, Mods.currentModDirectory);
				}
			}
		}
		catch (e:Dynamic)
