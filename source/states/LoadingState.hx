...
// weirdly enough, this was easy to backport??
// with the exception of having to make quite a few fixes
// and backporting other stuff
// but it does work
#if cpp
@:headerCode('
#include <iostream>
...
class LoadingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (Mods.currentModDirectory != null && Mods.currentModDirectory.length > 0)
		{
			...
			if (FileSystem.exists(scriptPath))
			{
				try
				{
					hscript = new FunkinHScript(scriptPath, false);
					hscript.setVariable('getLoaded', function() return loaded);
					hscript.setVariable('getLoadMax', function() return loadMax);
					hscript.setVariable('barBack', barBack);
					hscript.setVariable('bar', bar);
					hscript.setVariable('addBehindBar', function(obj:FlxBasic)
					{
						addBehindBar(obj);
					});
					hscript.execute(scriptPath, false);

					if (hscript.existsVariable('onCreate'))
					{
						hscript.executeFunc('onCreate', []);
				...
				{
					Lib.application.window.alert(Std.string(e), 'LoadingScreen.hx Error!');
				}
				if (hscript != null)
					hscript.destroy();
				hscript = null;
	...
	function addBehindBar(obj:FlxBasic)
	{
	...
	override function update(elapsed:Float)
	{
		...
		if (hscript != null)
		{
			if (hscript.existsVariable('onUpdate'))
				hscript.executeFunc('onUpdate', [elapsed]);
		}
		#end
	}
	...
	override function destroy()
	{
		if (hscript != null)
		{
			if (hscript.existsVariable('onDestroy'))
				hscript.executeFunc('onDestroy', []);
			hscript.destroy();
		}
		hscript = null;
		super.destroy();
	...
	public static function prepareToSong()
	{
		...
		new Future<Bool>(() ->
		{
		...
		}, isIntrusive).then((_) -> new Future<Bool>(() ->
			{
				...
				songsToPrepare.push('songs/$folder/Inst');

				var player1:String = song.player1;
				var player2:String = song.player2;
				var gfVersion:String = song.gfVersion;
				var prefixVocals:String = song.needsVoices ? 'songs/$folder/Voices' : null;
				if (gfVersion == null)
					gfVersion = 'gf';
				...
				{
					if (Paths.fileExists('$prefixVocals-Player.ogg') && Paths.fileExists('$prefixVocals-Opponent.ogg'))
					{
						songsToPrepare.push('$prefixVocals-Player');
						songsToPrepare.push('$prefixVocals-Opponent');
					}
					else if (Paths.fileExists('$prefixVocals.ogg'))
	...
	static function _threadFunc()
	{
		...
		for (song in songsToPrepare)
			initThread(() -> preloadSound(song, '', true, false), 'song $song');
		...
		try
		{
			...
			else
			{
				for (i in 0...10)
				{
					...
					if (Paths.fileExists('images/$img/spritemap$st.png'))
					{
	...
	static function preloadSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):Null<Sound>
	{
		...
		{
			if (#if sys FileSystem.exists(file) || #end Assets.exists(file, SOUND))
			{
				var sound:Sound = #if sys Sound.fromFile(file) #else Assets.getSound(file, false) #end;
				mutex.acquire();
