...
class PhillyStreets extends BaseStage
{
	...
	{
        StickerSubState.STICKER_SET = "sticker-set-2";
        StickerSubState.STICKER_PACK = "weekend";

		if (!ClientPrefs.data.lowQuality)
		{
			var skyImage = Paths.image('stages/phillyStreets/phillyStreets/phillySkybox');
			...
			var phillySkyline:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillySkyline', -545, -273, 0.2, 0.2);
			add(phillySkyline);
			darkenable.push(phillySkyline);

			var phillyForegroundCity:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyForegroundCity', 625, 94, 0.3, 0.3);
			add(phillyForegroundCity);
			darkenable.push(phillyForegroundCity);
		}

		var phillyConstruction:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyConstruction', 1800, 364, 0.7, 1);
		add(phillyConstruction);
		darkenable.push(phillyConstruction);

		var phillyHighwayLights:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyHighwayLights', 284, 305, 1, 1);
		add(phillyHighwayLights);
		darkenable.push(phillyHighwayLights);

		if (!ClientPrefs.data.lowQuality)
		{
			var phillyHighwayLightsLightmap:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyHighwayLights_lightmap', 284, 305, 1, 1);
		...
		var phillyHighway:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyHighway', 139, 209, 1, 1);
		add(phillyHighway);
		darkenable.push(phillyHighway);

		if (!ClientPrefs.data.lowQuality)
		{
			var phillySmog:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillySmog', -6, 245, 0.8, 1);
			add(phillySmog);
			darkenable.push(phillySmog);

			for (i in 0...2)
			{
				var car:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyCars', 1200, 818, 0.9, 1, ['car1', 'car2', 'car3', 'car4'], false);
				add(car);
				switch (i)
				{
					case 0:
			...
			phillyTraffic = new BGSprite('stages/phillyStreets/phillyStreets/phillyTraffic', 1840, 608, 0.9, 1, ['redtogreen', 'greentored'], false);
			add(phillyTraffic);
			darkenable.push(phillyTraffic);

			var phillyTrafficLightmap:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyTraffic_lightmap', 1840, 608, 0.9, 1);
		...
		var phillyForeground:BGSprite = new BGSprite('stages/phillyStreets/phillyStreets/phillyForeground', 88, 317, 1, 1);
	...
	override function createPost()
	{
		...
		spraycanPile = new BGSprite('stages/phillyStreets/SpraycanPile', 920, 1045, 1, 1);
		precache();
		add(spraycanPile);
	...
	function precache()
	{
		...
		function precacheCasing()
		{
			...
			if (!ClientPrefs.data.lowQuality)
			{
				casingFrames = Paths.getSparrowAtlas('stages/phillyStreets/PicoBullet'); // precache
		...
		// createPost runs before PlayState.generateSong(), so noteTypes is not populated yet.
		createCan();
		precacheCasing();
