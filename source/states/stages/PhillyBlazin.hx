...
class PhillyBlazin extends BaseStage
{
	...
	override function create()
	{
		...
		{
			var skyImage = Paths.image('stages/phillyStreets/phillyBlazin/skyBlur');
			...
			skyAdditive = new BGSprite('stages/phillyStreets/phillyBlazin/skyBlur', -600, -175, 0.0, 0.0);
			setupScale(skyAdditive);
			skyAdditive.visible = false;
			add(skyAdditive);

			lightning = new BGSprite('stages/phillyStreets/phillyBlazin/lightning', -50, -300, 0.0, 0.0, ['lightning0'], false);
			setupScale(lightning);
			lightning.visible = false;
			add(lightning);
		}

		var phillyForegroundCity:BGSprite = new BGSprite('stages/phillyStreets/phillyBlazin/streetBlur', -600, -175, 0.0, 0.0);
		setupScale(phillyForegroundCity);
		add(phillyForegroundCity);

		if (!ClientPrefs.data.lowQuality)
		{
			foregroundMultiply = new BGSprite('stages/phillyStreets/phillyBlazin/streetBlur', -600, -175, 0.0, 0.0);
		...
		if (_song.gameOverSound == null || _song.gameOverSound.trim().length < 1)
			deathSoundName = 'fnf_loss_sfx-pico-gutpunch';
