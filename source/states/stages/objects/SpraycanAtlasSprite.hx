...
class SpraycanAtlasSprite extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0)
	{
		...
		Paths.loadAnimateAtlas(canAtlas, 'stages/phillyStreets/spraycanAtlas');
		...
		canAtlas.antialiasing = ClientPrefs.data.globalAntialiasing;
		add(canAtlas);

		explosion = new FlxSprite(x - 25, y - 450);
		explosion.frames = Paths.getSparrowAtlas('stages/phillyStreets/spraypaintExplosionEZ');
		...
		explosion.antialiasing = ClientPrefs.data.globalAntialiasing;
		add(explosion);
	}
