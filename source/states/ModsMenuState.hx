...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		var path:String = Paths.mods('$folder/settings.json');
		...
		var isPixel = false;
		var path:String = Paths.mods('$folder/pack.png');
		if (!FileSystem.exists(path))
		{
			path = Paths.mods('$folder/pack-pixel.png');
			isPixel = true;
		}

		var bmp:FlxGraphic = null;
		if (FileSystem.exists(path))
			bmp = Paths.cacheBitmap(path, BitmapData.fromFile(path));
