...
class ReflectionFunctions
{
	...
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "getPropertyFromClass", function(classVar:String, variable:String, ?allowMaps:Bool = false)
		{
			...
			if (myClass == null)
			{
				LuaUtils.luaTrace(lua, 'getPropertyFromClass: Class $classVar not found', false, false, FlxColor.RED);
				return null;
			}
		...
		Lua_helper.add_callback(lua, "setPropertyFromClass",
			function(classVar:String, variable:String, value:Dynamic, ?allowMaps:Bool = false, ?allowInstances:Bool = false)
			{
				...
				if (myClass == null)
				{
					LuaUtils.luaTrace(lua, 'setPropertyFromClass: Class $classVar not found', false, false, FlxColor.RED);
					return null;
				}
		...
		Lua_helper.add_callback(lua, "getPropertyFromGroup", function(group:String, index:Int, variable:Dynamic, ?allowMaps:Bool = false)
		{
			...
			if (groupOrArray != null)
			{
				switch (Type.typeof(groupOrArray))
				{
					case TClass(Array): // Is Array
						...
						LuaUtils.luaTrace(lua, 'getPropertyFromGroup: Object #$index from group: $group doesn\'t exist!', false, false, FlxColor.RED);
			...
			LuaUtils.luaTrace(lua, 'getPropertyFromGroup: Group/Array $group doesn\'t exist!', false, false, FlxColor.RED);
			return null;
		});
		Lua_helper.add_callback(lua, "setPropertyFromGroup",
			function(group:String, index:Int, variable:Dynamic, value:Dynamic, ?allowMaps:Bool = false, ?allowInstances:Bool = false)
			{
				...
				if (realObject != null)
				{
					...
					LuaUtils.luaTrace(lua, 'setPropertyFromGroup: Group/Array $group doesn\'t exist!', false, false, FlxColor.RED);
				return value;
			});
		Lua_helper.add_callback(lua, "addToGroup", function(group:String, tag:String, ?index:Int = -1)
		{
			...
			{
				LuaUtils.luaTrace(lua, 'addToGroup: Object $tag is not valid!', false, false, FlxColor.RED);
				return;
			}
			...
			if (groupOrArray == null)
			{
				LuaUtils.luaTrace(lua, 'addToGroup: Group/Array $group is not valid!', false, false, FlxColor.RED);
				return;
			}
		...
		Lua_helper.add_callback(lua, "removeFromGroup", function(group:String, ?index:Int = -1, ?tag:String = null, ?destroy:Bool = true)
		{
			...
			if (tag != null)
			{
				...
				if (obj == null || obj.destroy == null)
				{
					LuaUtils.luaTrace(lua, 'removeFromGroup: Object $tag is not valid!', false, false, FlxColor.RED);
					return;
				}
			}
			...
			if (groupOrArray == null)
			{
				LuaUtils.luaTrace(lua, 'removeFromGroup: Group/Array $group is not valid!', false, false, FlxColor.RED);
				return;
			}
		...
		Lua_helper.add_callback(lua, "createInstance", function(variableToSave:String, className:String, ?args:Array<Dynamic>)
		{
			...
			if (MusicBeatState.getVariables().get(variableToSave) == null)
			{
				...
				if (myType == null)
				{
					LuaUtils.luaTrace(lua, 'createInstance: Class $className not found', false, false, FlxColor.RED);
					return false;
				}
				...
				if (obj != null)
					...
					LuaUtils.luaTrace(lua, 'createInstance: Failed to create $variableToSave, arguments are possibly wrong.', false, false, FlxColor.RED);

				return (obj != null);
			}
			else
				LuaUtils.luaTrace(lua, 'createInstance: Variable $variableToSave is already being used and cannot be replaced!', false, false, FlxColor.RED);
			return false;
		});
		Lua_helper.add_callback(lua, "addInstance", function(objectName:String, ?inFront:Bool = false)
		{
			...
			if (savedObj != null)
			{
				...
				else
				{
					...
					{
						var gameOverBoyfriend:Dynamic = gameOver.scriptGet('boyfriend');
						if (gameOverBoyfriend != null)
							gameOver.insert(gameOver.members.indexOf(gameOverBoyfriend), obj);
					}
				}
			}
			else
				LuaUtils.luaTrace(lua, 'addInstance: Can\'t add what doesn\'t exist~ ($objectName)', false, false, FlxColor.RED);
		});
